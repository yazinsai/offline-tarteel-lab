"""Phoneme CTC beam decode + Quran matcher (joint model/runtime variant).

Bounded prefix beam over ONNX log-produces several transcript hypotheses per clip, then
runs the same Levenshtein/span matcher as reference_phoneme_v4 with slightly widened search.
Audio path only at inference — no manifest/filename metadata for labels.
"""
