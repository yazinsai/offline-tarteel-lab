"""Phoneme CTC beam + matcher with dual acoustic preprocessing (two-pass ONNX decode).

Runs the shipped phoneme ONNX twice on identical audio using two deterministic pre-emphasis
coefficients, unions CTC hypotheses, then selects the matcher-best hit. Bounded CPU
(seconds forward passes = 2; beam caps unchanged versus joint02). No manifest metadata.
"""
...
# Comma-separated pre-emphasis coeffs for pooled acoustic hypotheses (ASCII order).
_DUAL_PRE_RAW = os.getenv("PHONEME_DUAL_PREEMPH", "0.97,0.915,0.865").strip()
DUAL_PREEMPH_COEFFS: tuple[float, ...] = tuple(
    float(x.strip()) for x in _DUAL_PRE_RAW.split(",") if x.strip()
) or (0.97, 0.915, 0.865)
# Secondary branch CMVN on log-mel: "mean" matches training-style norm; optional "rms" divides
# by RMS per frame for loudness-normalized second pass before per-bin whitening.
_SECOND_CMVN_RAW = os.getenv("PHONEME_DUAL_CMVN", "mean").strip()
CMVN_MODES: tuple[str, ...] = tuple(
    m.strip().lower()
    for m in _SECOND_CMVN_RAW.split(",")
    if m.strip()
) or ("mean", "rms_mean")
ACOUSTIC_POOL_CAP = int(os.getenv("PHONEME_DUAL_POOL_CAP", "14"))
...
def _cmvn_logmel(mel: np.ndarray, mode: str) -> np.ndarray:
    m = mode.strip().lower()
    if m == "rms_mean":
        frame_rms = np.sqrt(np.mean(mel * mel, axis=0, keepdims=True)) + 1e-10
        mel = mel / frame_rms
    return (mel - mel.mean(axis=1, keepdims=True)) / (mel.std(axis=1, keepdims=True) + 1e-10)
...
def _compute_logprobs(
    ...
    *,
    ...
    pe = float(preemph)
    x = np.append(audio[0], audio[1:] - pe * audio[:-1])
    ...
    mel = np.log(mel + 1e-5)
    mel = _cmvn_logmel(mel, cmvn_mode)
...
def _hypotheses_from_logprobs(logprobs: np.ndarray) -> list[str]:
...
def predict(audio_path: str) -> dict:
    ...
    seen: set[str] = set()
    hyps: list[str] = []
    lp_last: np.ndarray | None = None
    # Cross product of CMVN modes × pre-emphasis coeffs, bounded by ACOUSTIC_POOL_CAP.
    for cmvn in CMVN_MODES:
        for coef in DUAL_PREEMPH_COEFFS:
            lp = _compute_logprobs(audio, _onnx_session, preemph=coef, cmvn_mode=cmvn)
            lp_last = lp
            for hyp in _hypotheses_from_logprobs(lp):
                h = hyp.strip()
                if not h or h in seen:
                    continue
                seen.add(h)
                hyps.append(hyp)
                if len(hyps) >= ACOUSTIC_POOL_CAP:
                    break
            if len(hyps) >= ACOUSTIC_POOL_CAP:
                break
        if len(hyps) >= ACOUSTIC_POOL_CAP:
            break
    if not hyps:
        lp0 = _compute_logprobs(
            audio,
            _onnx_session,
            preemph=DUAL_PREEMPH_COEFFS[0],
            cmvn_mode=CMVN_MODES[0],
        )
        lp_last = lp0
        hyps = _hypotheses_from_logprobs(lp0)
    ...
    for hyp in hyps:
        ...
        if float(top[0]["score"]) > best_rank:
            ...
            best_transcript = hyp
    if not best_hit:
        assert lp_last is not None
        fallback = _labels_to_phoneme_string(_greedy_decode_ids(lp_last))
