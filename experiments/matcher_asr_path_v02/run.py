"""Phoneme CTC + Quran matcher variant 02: stronger fragment-aware ranking (no label leakage).

Same ONNX decode path as reference_phoneme_v4; matcher blends global Levenshtein ratio with
semi-global fragment scores for all candidate lengths ≥6 chars, and augments span hypotheses
similarly. Reuses sibling reference_phoneme_v4/quran_phonemes.json (no duplicate DB).
"""
...
# Share the committed Quran phoneme table with the port experiment (avoid a second 1.5MB JSON).
_QURAN_JSON = _DIR.parent / "reference_phoneme_v4" / "quran_phonemes.json"
...
# Keep TOP_K modestly higher than reference (14) for rerank headroom; span cost matches v4 (5).
TOP_K_LEVENSHTEIN = int(os.getenv("PHONEME_LM_TOP_K", "15"))
TOP_SURAHS = int(os.getenv("PHONEME_LM_TOP_SURAHS", "28"))
MAX_SPAN = int(os.getenv("PHONEME_LM_MAX_SPAN", "5"))

# Blend weights for fragment vs global ratio (matcher-only; not streaming smoke knobs).
_FRAG_BLEND = float(os.getenv("PHONEME_MATCHER_FRAG_BLEND", "0.42"))
...
def _frag_max(no_space_text: str, verse: dict) -> float:
    ref_ns = verse.get("_phonemes_joined_ns", "") or ""
    if not ref_ns:
        return 0.0
...
def _blend_with_fragment(raw: float, frag: float, *, no_space_text: str, verse: dict) -> float:
    """Upgrade raw Levenshtein with fragment evidence when query looks like a prefix fragment."""
    if len(no_space_text) < 6:
        return raw
    ref_ns = verse.get("_phonemes_joined_ns", "") or ""
    if not ref_ns or len(no_space_text) >= len(ref_ns) * 0.92:
        return raw
    blended = (1.0 - _FRAG_BLEND) * raw + _FRAG_BLEND * frag
    # Preserve the legacy "boost when frag explains better" shape but always add some frag signal.
    legacy_boost = raw + max(0.0, frag - raw) * 0.72
    return max(raw, blended, legacy_boost)
...
def _match_phoneme_text(phoneme_text: str, top_k: int = 10) -> list[dict]:
    ...
    # Cheap rerank: semi-global fragment only on the ratio-shortlist head and only when the query
    # could be a strict prefix of the verse (same cutoff idea as reference v4, plus a fixed blend).
    _head = min(len(scored), 72)
    for i in range(_head):
        verse, raw, _ = scored[i]
        ref_ns = verse.get("_phonemes_joined_ns", "") or ""
        if (
            len(no_space_text) < 8
            or not ref_ns
            or len(no_space_text) >= len(ref_ns) * 0.8
        ):
            continue
        frag = _frag_max(no_space_text, verse)
        combined = _blend_with_fragment(raw, frag, no_space_text=no_space_text, verse=verse)
        scored[i] = [verse, raw, combined]
    scored.sort(key=lambda x: x[2], reverse=True)
...
def _ensure_loaded() -> None:
    ...
    if _onnx_session is not None and _verses is not None:
        return
    if not _QURAN_JSON.is_file():
        raise FileNotFoundError(f"missing shared Quran phoneme DB: {_QURAN_JSON}")
