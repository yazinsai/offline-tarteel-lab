"""Reference-port v41: wider bounded CTC prefix beam + joint02 matcher stack.

Default beam 10x10 with up to 8 hypotheses (vs joint02 6x8x4) to expand decode search under a
documented CPU cap; hypothesis selection remains matcher-argmax over decoded strings (no manifest
metadata). Reuses joint02 ONNX cache and Quran table path.
"""
...
_QURAN_JSON = _DIR.parent / "phoneme_matcher_joint02" / "quran_phonemes.json"
...
# Wider prefix beam vs phoneme_matcher_joint02 defaults (6×8×4):bounded CPU for v41 reference-port decode search.
...
def _iter_beam_labelings(logprobs: np.ndarray) -> list[tuple[tuple[int, ...], str]]:
    """Greedy labeling first, then unique prefix-beam labelings (bounded by MAX_HYPOTHESES)."""
    out: list[tuple[tuple[int, ...], str]] = []
    seen: set[tuple[int, ...]] = set()
    greedy = _greedy_decode_ids(logprobs)
    if greedy not in seen:
        seen.add(greedy)
        gs = _labels_to_phoneme_string(greedy)
        if gs.strip():
            out.append((greedy, gs))
    if BEAM_WIDTH <= 1:
        return out
...
def predict(audio_path: str) -> dict:
    ...
    best_transcript = ""
    best_rank = -1.0
    for _, hyp in _iter_beam_labelings(logprobs):
