"""ASR-focused variant: wider CTC prefix beam over the same phoneme ONNX as joint02.

Decode-only widening vs phoneme_matcher_joint02 defaults (beam 10 / top-10 symbols / 6 hyps).
Matcher path is unchanged from joint02; labels still come only from audio-derived phonemes.
CPU wall-time per clip increases roughly linearly with beam state; intended for offline eval caps.
"""
...
# Wider beam than joint02 (6/8/4) to stress-test decode search under offline CPU caps.
