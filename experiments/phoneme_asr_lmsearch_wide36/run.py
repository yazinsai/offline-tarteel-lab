"""Phoneme CTC beam decode + widened Quran matcher (model_only retrieval-head variant).

Clone of phoneme_matcher_joint02 with larger default matcher search caps (TOP_K / surahs /
multi-ayah spans) targeting better first-ayah accuracy from the same shipped ONNX logits.
Labels come only from phoneme decode vs Quran JSON; no corpus metadata leakage.
"""
...
# Widen phoneme Quran retrieval/search after the same ONNX CTC decode (model_only ASR-head probe).
TOP_K_LEVENSHTEIN = int(os.getenv("PHONEME_LM_TOP_K", "24"))
TOP_SURAHS = int(os.getenv("PHONEME_LM_TOP_SURAHS", "48"))
MAX_SPAN = int(os.getenv("PHONEME_LM_MAX_SPAN", "7"))
