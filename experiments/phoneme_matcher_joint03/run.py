"""Phoneme CTC beam decode + Quran matcher — joint03 (decode expansion vs joint02).

Same matcher + ONNX as phoneme_matcher_joint02, but expands the CTC beam search (more
frame symbols, wider beam, slightly more retained hypotheses) before matcher argmax.
Hypothesis choice remains max matcher score across hyps — no ASR+logprob fusion rerank.
CPU/time note: ~1.3–1.8× joint02 decode cost on typical clips (env caps below).
"""
...
# Decode expansion vs joint02 (6/8/4): wider search before matcher pick; still bounded for CPU.
BEAM_WIDTH = int(os.getenv("PHONEME_CTC_BEAM_WIDTH", "10"))
BEAM_TOP_SYMBOLS = int(os.getenv("PHONEME_CTC_TOP_SYMBOLS", "11"))
