"""Dual-view acoustic front-end + joint02 beam/matcher (model_only.46.03).

Two ONNX passes per clip: same mel recipe as joint02 but with different preemphasis on the
delta term (defaults 0.97 vs raw waveform / no preemph). Runs bounded CTC beam on each log-prob
tensor, unions up to ten distinct transcript hypotheses, then uses the same Quran matcher as
phoneme_matcher_joint02.
~2x ONNX forward and up to doubled beam expansion; no manifest/filename labels."""
...
PREEMPH_PRIMARY = float(os.getenv("DUALPREEMPH_PRIMARY", "0.97"))
PREEMPH_SECOND = float(os.getenv("DUALPREEMPH_SECOND", "0.0"))
...
def _compute_logprobs(
    audio: np.ndarray, session: ort.InferenceSession, *, preemph: float = 0.97
) -> np.ndarray:
    if preemph <= 0.0:
        x = audio.astype(np.float32, copy=False)
    else:
        x = np.append(audio[0], audio[1:] - preemph * audio[:-1])
...
def predict(audio_path: str) -> dict:
    ...
    lp_a = _compute_logprobs(audio, _onnx_session, preemph=PREEMPH_PRIMARY)
    lp_b = _compute_logprobs(audio, _onnx_session, preemph=PREEMPH_SECOND)
    hyps: list[str] = []
    seen: set[str] = set()
    cap = int(os.getenv("DUALPREEMPH_MAX_UNION_HYPS", "10"))
    for hyp in _hypotheses_from_logprobs(lp_a) + _hypotheses_from_logprobs(lp_b):
        if hyp in seen:
            continue
        seen.add(hyp)
        hyps.append(hyp)
