"""Phoneme CTC beam + matcher (lightweight dual-preemph, model_only 46.x).

Two pre-emphasis frontends each run the shipped phoneme ONNX; frame log-probs are averaged
in probability space before the same bounded CTC prefix beam and Quran matcher stack as
phoneme_matcher_joint02. No manifest/path label leakage.
"""
...
# Wider CTC search than joint02: compensates for averaged dual-forward log-probs being smoother.
BEAM_WIDTH = int(os.getenv("PHONEME_CTC_BEAM_WIDTH", "11"))
BEAM_TOP_SYMBOLS = int(os.getenv("PHONEME_CTC_TOP_SYMBOLS", "12"))
MAX_HYPOTHESES = int(os.getenv("PHONEME_MAX_HYPOTHESES", "6"))
...
def _forward_logprobs(audio: np.ndarray, session: ort.InferenceSession, *, preemph_coef: float) -> np.ndarray:
    x = np.append(audio[0], audio[1:] - preemph_coef * audio[:-1])
...
def _compute_logprobs(audio: np.ndarray, session: ort.InferenceSession) -> np.ndarray:
    """Average two acoustic views in probability space (two ONNX forwards), then shared CTC beam."""
    a = float(os.getenv("LIGHTWEIGHT_DUAL_PREEMPH_A", "0.97"))
    b = float(os.getenv("LIGHTWEIGHT_DUAL_PREEMPH_B", "0.92"))
    lp_a = _forward_logprobs(audio, session, preemph_coef=a)
    lp_b = _forward_logprobs(audio, session, preemph_coef=b)
    return np.logaddexp(lp_a, lp_b) - np.log(2.0)
