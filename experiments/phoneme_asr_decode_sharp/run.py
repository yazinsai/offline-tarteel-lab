"""Phoneme CTC decode sharpen + wider beam; matcher matches promoted phoneme_matcher_joint02.

model_only probe: same ONNX + Quran matcher/search as joint02, but wider bounded beam and
log-prob sharpening before CTC beam expansion to improve transcripts from audio only.
"""
...
# Wider but still bounded beam (CPU cap); defaults above joint02 for decode-focused probe.
...
# Sharpen log-probs before beam (>>1 peakier posteriors); tunable via env.
LOGPROB_SHARPEN = float(os.getenv("PHONEME_LOGPROB_SHARPEN", "1.07"))
...
def predict(audio_path: str) -> dict:
    ...
    if LOGPROB_SHARPEN != 1.0:
        logprobs = logprobs * LOGPROB_SHARPEN
