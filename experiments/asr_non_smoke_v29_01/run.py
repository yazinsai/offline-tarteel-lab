"""Non-smoke ASR experiment (model_only v29.01).

Same ONNX + matcher scaffolding as phoneme_matcher_joint02 with ASR-only changes:
preemphasis-ensemble logits (fuse multiple coefs in probability space) and wider
default CTC prefix beam before hypothesis matching (no corpus metadata leakage).
"""
...
# Wider default beam vs joint02 decode path; narrower env override preserves CPU budgets.
_BEAM_ENV = os.getenv("PHONEME_CTC_BEAM_WIDTH", "").strip()
BEAM_WIDTH = int(_BEAM_ENV or "8")
_BEAM_SYM_ENV = os.getenv("PHONEME_CTC_TOP_SYMBOLS", "").strip()
BEAM_TOP_SYMBOLS = int(_BEAM_SYM_ENV or "11")
_MAXH_ENV = os.getenv("PHONEME_MAX_HYPOTHESES", "").strip()
MAX_HYPOTHESES = int(_MAXH_ENV or "8")
...
def _forward_mel(session: ort.InferenceSession, mel: np.ndarray) -> np.ndarray:
    """Run ONNX graph on precomputed normalized log-mels (channels x frames)."""
...
def _logprobs_single_preemph(audio: np.ndarray, coef: float, session: ort.InferenceSession) -> np.ndarray:
    x = np.append(audio[0], audio[1:] - coef * audio[:-1])
    ...
    return _forward_mel(session, mel)


def _compute_logprobs(audio: np.ndarray, session: ort.InferenceSession) -> np.ndarray:
    raw = os.getenv("PHONEME_PREEM_COEF_ENSEMBLE", "").strip()
    coeffs = tuple(float(x) for x in raw.split(",")) if raw else (0.97, 0.92, 0.87)
    probs_sum: np.ndarray | None = None
    for coef in coeffs:
        lp = _logprobs_single_preemph(audio, coef, session)
        row_max = lp.max(axis=1, keepdims=True)
        p = np.exp(lp - row_max)
    ...
    fused = probs_sum / float(len(coeffs))
    fused /= fused.sum(axis=1, keepdims=True) + 1e-15
    temp = float(os.getenv("PHONEME_DECODE_TEMP", "0.94"))
    if abs(temp - 1.0) > 1e-6 and temp > 0:
        sharp = np.power(np.clip(fused, 1e-15, None), 1.0 / temp)
        sharp /= sharp.sum(axis=1, keepdims=True) + 1e-15
        fused = sharp
    return np.log(np.clip(fused, 1e-20, None))
...
def predict(audio_path: str) -> dict:
    ...
    hyps = _hypotheses_from_logprobs(logprobs)
