...
def test_smoke_default_smoothing_window_variant_05(monkeypatch, tmp_path):
    ...
    ratio = out["streaming"]["lock_confidence"]
    assert out["streaming"]["lock_confidence_smoothed"] == mod._lock_confidence_smoothed(
        ratio,
        mod._DEFAULT_SMOOTHING_WINDOW,
    )
...
def test_smoke_smoothing_window_env_override(monkeypatch, tmp_path):
    ...
    mod = _load_smoke_run()
    audio = tmp_path / "smooth-env.wav"
    audio.write_bytes(b"")
    out = mod.predict(str(audio))
    assert out["streaming"]["smoothing_window"] == 8
    ratio = out["streaming"]["lock_confidence"]
    assert out["streaming"]["lock_confidence_smoothed"] == mod._lock_confidence_smoothed(ratio, 8)
